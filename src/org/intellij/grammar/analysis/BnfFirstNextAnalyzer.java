...
public class BnfFirstNextAnalyzer
{
	...
	private Map<BnfExpression, BnfExpression> calcNextInner(@NotNull BnfExpression targetExpression, Map<BnfExpression, BnfExpression> result,
	...
	private Set<BnfExpression> calcSequenceFirstInner(List<BnfExpression> expressions, final Set<BnfExpression> result,
			final Set<BnfExpression> visited)
	{
	...
	public Set<BnfExpression> calcFirstInner(BnfExpression expression, Set<BnfExpression> result, Set<BnfExpression> visited,
			@Nullable List<BnfExpression> forcedNext)
	{
		...
		else if(expression instanceof BnfReferenceOrToken)
		{
			...
			if(rule != null)
			{
				...
				if(myPublicRuleOpaque && !ParserGeneratorUtil.Rule.isPrivate(rule) || !visited.add(ruleExpression))
				{
		...
		else if(expression instanceof BnfPredicate)
		{
			...
			boolean skip = predicateExpression instanceof BnfSequence && ((BnfSequence) predicateExpression).getExpressionList().size() > 1; // todo
			// calc min length ?
