...
public class ParserGeneratorUtil
{
	...
	public static BnfExpression getTrivialNodeChild(PsiElement element)
	{
		...
		else if(element.getFirstChild() == element.getLastChild() && (element instanceof BnfChoice || element instanceof BnfSequence || element
				instanceof BnfExpression))
		{
			child = element.getFirstChild();
		}
		return child instanceof BnfExpression && !(child instanceof BnfLiteralExpression || child instanceof BnfReferenceOrToken) ? (BnfExpression)
				child : null;
	}
	...
	public static String toDisplayOrConstantName(String name, boolean constant)
	{
		...
		{
			strings[i] = constant ? strings[i].toUpperCase() : strings[i].toLowerCase();
		}
	...
	public static Collection<BnfExpression> getSortedTokens(Set<PsiElement> accessors)
	{
		...
		for(PsiElement tree : accessors)
		{
			if(!(tree instanceof BnfExpression))
			{
				continue;
			}
