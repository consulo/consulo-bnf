...
public class ExpressionGeneratorHelper
{
	...
	public static void generateExpressionRoot(ExpressionHelper.ExpressionInfo info, ParserGenerator g)
	{
		...
		for(BnfRule rule : info.priorityMap.keySet())
		{
			...
			if(list == null)
			{
				opCalls.put(opCall, list = new ArrayList<OperatorInfo>(2));
			}
			list.add(operator);
		}
		...
		for(String opCall : sortedOpCalls)
		{
			OperatorInfo operator = ContainerUtil.getFirstItem(findOperators(opCalls.get(opCall), OperatorType.BINARY, OperatorType.N_ARY,
			...
			String tailCall = operator.tail == null ? null : g.generateNodeCall(operator.rule, operator.tail, getNextName(operator.rule.getName(),
					1), ConsumeType.DEFAULT);
			if(operator.type == OperatorType.BINARY)
			{
				g.out("result_ = report_error_(builder_, " + methodName + "(builder_, level_, " + (rightAssociative ? argPriority - 1 : argPriority)
						+ "));");
				if(tailCall != null)
				{
		...
		for(String opCall : sortedOpCalls)
		{
			for(OperatorInfo operator : opCalls.get(opCall))
			{
				...
				else if(operator.type == OperatorType.PREFIX)
				{
					...
					String elementType = ParserGeneratorUtil.getElementType(operator.rule);
					String tailCall = operator.tail == null ? null : g.generateNodeCall(operator.rule, operator.tail,
							getNextName(operator.rule.getName(), 1), ConsumeType.DEFAULT);
	...
	public static ExpressionHelper.ExpressionInfo getInfoForExpressionParsing(ExpressionHelper expressionHelper, BnfRule rule)
	{
		...
		if(expressionInfo != null && (operatorInfo == null || operatorInfo.type != OperatorType.ATOM && operatorInfo.type != OperatorType.PREFIX))
		{
	...
	@Nullable
	public static ConsumeType fixForcedConsumeType(@NotNull ExpressionHelper expressionHelper, @NotNull BnfRule rule, @Nullable BnfExpression node,
			@Nullable ConsumeType defValue)
	{
		...
		if(operatorInfo != null)
		{
			if(node == null)
			{
				return operatorInfo.type == OperatorType.PREFIX || operatorInfo.type == OperatorType.ATOM ? CONSUME_TYPE_OVERRIDE : null;
			}
